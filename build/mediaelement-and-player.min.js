...
a.container;b.bind(mejs.MediaFeatures.fullScreenEventName,function(){if(mejs.MediaFeatures.isFullScreen()){a.isNativeFullScreen=true;a.setControlsSize()}else{a.isNativeFullScreen=false;a.exitFullScreen()}})}var d=this,g=f('<div class="mejs-button mejs-fullscreen-button"><button type="button" aria-controls="'+d.id+'" aria-live="polite"><span class="visuallyhidden">'+d.options.fullscreenText+"</span></button></div>").appendTo(c);d.container.parent().on("focus","button.guard",function(){mejs.MediaFeatures.hasTrueNativeFullScreen&&
mejs.MediaFeatures.isFullScreen()||a.isFullScreen?g.trigger("click"):d.container.parent().find("button.guard").remove()});if(d.media.pluginType==="native"||!d.options.usePluginFullScreen&&!mejs.MediaFeatures.isFirefox)g.click(function(){var j=mejs.MediaFeatures.hasTrueNativeFullScreen&&mejs.MediaFeatures.isFullScreen()||a.isFullScreen;d.container.find("#btnplaypause").focus();if(j){a.exitFullScreen();d.container.parent().find("button.guard").remove()}else{a.enterFullScreen();d.container.before('<button class="guard visuallyhidden">Quitter le mode plein \u00e9cran</button>');
d.container.after('<button class="guard visuallyhidden">Quitter le mode plein \u00e9cran</button>')}});else{var k=null;c=function(){var j=document.createElement("x"),p=document.documentElement,h=window.getComputedStyle;if(!("pointerEvents"in j.style))return false;j.style.pointerEvents="auto";j.style.pointerEvents="x";p.appendChild(j);h=h&&h(j,"").pointerEvents==="auto";p.removeChild(j);return!!h}();console.log("supportsPointerEvents",c);if(c&&!mejs.MediaFeatures.isOpera){var i=false,l=function(){if(i){m.hide();
o.hide();r.hide();g.css("pointer-events","");d.controls.css("pointer-events","");i=false}},m=f('<div class="mejs-fullscreen-hover" />').appendTo(d.container).mouseover(l),o=f('<div class="mejs-fullscreen-hover"  />').appendTo(d.container).mouseover(l),r=f('<div class="mejs-fullscreen-hover"  />').appendTo(d.container).mouseover(l),t=function(){var j={position:"absolute",top:0,left:0};m.css(j);o.css(j);r.css(j);m.width(d.container.width()).height(d.container.height()-d.controls.height());j=g.offset().left-
d.container.offset().left;fullScreenBtnWidth=g.outerWidth(true);o.width(j).height(d.controls.height()).css({top:d.container.height()-d.controls.height()});r.width(d.container.width()-j-fullScreenBtnWidth).height(d.controls.height()).css({top:d.container.height()-d.controls.height(),left:j+fullScreenBtnWidth})};f(document).resize(function(){t()});g.mouseover(function(){if(!d.isFullScreen){var j=g.offset(),p=a.container.offset();e.positionFullscreenButton(j.left-p.left,j.top-p.top,false);g.css("pointer-events",
...
mejs.MediaFeatures.isFullScreen()||d.isFullScreen)&&j.keyCode==27)a.exitFullScreen()})}},enterFullScreen:function(){var a=this;if(!(a.media.pluginType!=="native"&&(mejs.MediaFeatures.isFirefox||a.options.usePluginFullScreen))){docStyleOverflow=document.documentElement.style.overflow;document.documentElement.style.overflow="hidden";normalHeight=a.container.height();normalWidth=a.container.width();if(a.media.pluginType==="native")if(mejs.MediaFeatures.hasTrueNativeFullScreen){mejs.MediaFeatures.requestFullScreen(a.container[0]);
