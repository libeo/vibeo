...
tooltip:"Help",features:["playpause","current","progress","duration","tracks","volume","fullscreen"],isVideo:true,enableKeyboard:true,pauseOtherPlayers:true,keyActions:[{keys:[32,179],action:function(a,c){c.paused||c.ended?c.play():c.pause()}},{keys:[38],action:function(a,c){c.setVolume(Math.min(c.volume+0.1,1))}},{keys:[40],action:function(a,c){c.setVolume(Math.max(c.volume-0.1,0))}},{keys:[37,227],action:function(a,c){if(!isNaN(c.duration)&&c.duration>0){a.isVideo&&a.showControls();var b=Math.max(c.currentTime-
...
c.isiPhone&&a.options.iPhoneUseNativeControls){a.$media.attr("controls","controls");if(c.isiPad&&a.media.getAttribute("autoplay")!==null){a.media.load();a.media.play()}}else if(!(c.isAndroid&&a.AndroidUseNativeControls)){a.$media.removeAttr("controls");a.id="mep_"+mejs.mepIndex++;a.container=f('<div id="'+a.id+'" class="mejs-container"><div class="mejs-inner"><div class="mejs-mediaelement"></div><div class="mejs-button mejs-tooltip"><button type="button" aria-live="polite"><span class="visuallyhidden">'+
a.options.tooltip+'</span></button><span class="bubble-text">'+a.options.tooltip+'</span></div><div class="mejs-layers"></div><div class="mejs-controls"></div><div class="mejs-clear"></div></div></div>').addClass(a.$media[0].className).insertBefore(a.$media);a.container.addClass((c.isAndroid?"mejs-android ":"")+(c.isiOS?"mejs-ios ":"")+(c.isiPad?"mejs-ipad ":"")+(c.isiPhone?"mejs-iphone ":"")+(a.isVideo?"mejs-video ":"mejs-audio "));if(c.isiOS){c=a.$media.clone();a.container.find(".mejs-mediaelement").append(c);
a.$media.remove();a.$node=a.$media=c;a.node=a.media=c[0]}else a.container.find(".mejs-mediaelement").append(a.$media);setTimeout(function(){var d=a.container.find("video");d.attr("tabindex")==0&&d.removeAttr("tabindex")},200);a.controls=a.container.find(".mejs-controls");a.layers=a.container.find(".mejs-layers");c=e.substring(0,1).toUpperCase()+e.substring(1);a.width=a.options[e+"Width"]>0||a.options[e+"Width"].toString().indexOf("%")>-1?a.options[e+"Width"]:a.media.style.width!==""&&a.media.style.width!==
null?a.media.style.width:a.media.getAttribute("width")!==null?a.$media.attr("width"):a.options["default"+c+"Width"];a.height=a.options[e+"Height"]>0||a.options[e+"Height"].toString().indexOf("%")>-1?a.options[e+"Height"]:a.media.style.height!==""&&a.media.style.height!==null?a.media.style.height:a.$media[0].getAttribute("height")!==null?a.$media.attr("height"):a.options["default"+c+"Height"];a.setPlayerSize(a.width,a.height);b.pluginWidth=a.height;b.pluginHeight=a.width}popuptooltip=function(){f(".bubble-text").show("fast");
f(".bubble-text").css("display","block")};hidetooltip=function(){f(".bubble-text").hide("fast")};f(".mejs-tooltip").mouseenter(popuptooltip);f(".mejs-tooltip").focusin(popuptooltip);f(".mejs-tooltip").mouseleave(hidetooltip);f(".mejs-tooltip").focusout(hidetooltip);mejs.MediaElement(a.$media[0],b)},showControls:function(a){var c=this;a=typeof a=="undefined"||a;if(!c.controlsAreVisible){if(a){c.controls.css("visibility","visible").stop(true,true).fadeIn(200,function(){c.controlsAreVisible=true});c.container.find(".mejs-control").css("visibility",
...
c.container.find(".mejs-control").stop(true,true).fadeOut(200,function(){f(this).css("visibility","hidden").css("display","block")})}else{c.controls.css("visibility","hidden").css("display","block");c.container.find(".mejs-control").css("visibility","hidden").css("display","block");c.controlsAreVisible=false}},controlsTimer:null,startControlsTimer:function(a){var c=this;a=typeof a!="undefined"?a:1500;c.killControlsTimer("start");c.controlsTimer=setTimeout(function(){c.hideControls();c.killControlsTimer("hide")},
a)},killControlsTimer:function(){if(this.controlsTimer!==null){clearTimeout(this.controlsTimer);delete this.controlsTimer;this.controlsTimer=null}},controlsEnabled:true,disableControls:function(){this.killControlsTimer();this.hideControls(false);this.controlsEnabled=false},enableControls:function(){this.showControls(false);this.controlsEnabled=true},meReady:function(a,c){var b=this,e=mejs.MediaFeatures,d=c.getAttribute("autoplay");d=!(typeof d=="undefined"||d===null||d==="false");var g;if(!b.created){b.created=
true;b.media=a;b.domNode=c;if(!(e.isAndroid&&b.options.AndroidUseNativeControls)&&!(e.isiPad&&b.options.iPadUseNativeControls)&&!(e.isiPhone&&b.options.iPhoneUseNativeControls)){b.buildposter(b,b.controls,b.layers,b.media);b.buildkeyboard(b,b.controls,b.layers,b.media);b.buildoverlays(b,b.controls,b.layers,b.media);b.findTracks();for(g in b.options.features){e=b.options.features[g];if(b["build"+e])try{b["build"+e](b,b.controls,b.layers,b.media)}catch(j){}}b.container.trigger("controlsready");b.setPlayerSize(b.width,
b.height);b.setControlsSize();f(".mejs-currenttime").resize(function(){b.setControlsSize()});if(b.isVideo){if(mejs.MediaFeatures.hasTouch)b.$media.bind("touchstart",function(){if(b.controlsAreVisible)b.hideControls(false);else b.controlsEnabled&&b.showControls(false)});else{(b.media.pluginType=="native"?b.$media:f(b.media.pluginElement)).click(function(){a.paused?a.play():a.pause()});b.container.bind("mouseenter mouseover focusin",function(){if(b.controlsEnabled)if(!b.options.alwaysShowControls){b.killControlsTimer("enter");
b.showControls()}}).bind("mousemove",function(){if(b.controlsEnabled){b.controlsAreVisible||b.showControls();b.options.alwaysShowControls||b.startControlsTimer(2500)}}).bind("mouseleave focusout",function(){b.controlsEnabled&&!b.media.paused&&!b.options.alwaysShowControls&&b.startControlsTimer(1E3)})}d&&!b.options.alwaysShowControls&&b.hideControls();b.options.enableAutosize&&b.media.addEventListener("loadedmetadata",function(i){if(b.options.videoHeight<=0&&b.domNode.getAttribute("height")===null&&
!isNaN(i.target.videoHeight)){b.setPlayerSize(i.target.videoWidth,i.target.videoHeight);b.setControlsSize();b.media.setVideoSize(i.target.videoWidth,i.target.videoHeight)}},false)}a.addEventListener("play",function(){for(var i=0,l=mejs.players.length;i<l;i++){var m=mejs.players[i];m.id!=b.id&&b.options.pauseOtherPlayers&&!m.paused&&!m.ended&&m.pause();m.hasFocus=false}b.hasFocus=true},false);a.addEventListener("focus",function(){for(var i=0,l=mejs.players.length;i<l;i++){var m=mejs.players[i];m.id!=
b.id&&b.options.pauseOtherPlayers&&!m.paused&&!m.ended&&m.pause();m.hasFocus=false}b.hasFocus=true});b.media.addEventListener("ended",function(){try{b.media.setCurrentTime(0)}catch(i){}b.media.pause();b.setProgressRail&&b.setProgressRail();b.setCurrentRail&&b.setCurrentRail();if(b.options.loop)b.media.play();else!b.options.alwaysShowControls&&b.controlsEnabled&&b.showControls()},false);b.media.addEventListener("loadedmetadata",function(){b.updateDuration&&b.updateDuration();b.updateCurrent&&b.updateCurrent();
if(!b.isFullScreen){b.setPlayerSize(b.width,b.height);b.setControlsSize()}},false);setTimeout(function(){b.setPlayerSize(b.width,b.height);b.setControlsSize()},50);f(window).resize(function(){b.isFullScreen||mejs.MediaFeatures.hasTrueNativeFullScreen&&document.webkitIsFullScreen||b.setPlayerSize(b.width,b.height);b.setControlsSize()});b.media.pluginType=="youtube"&&b.container.find(".mejs-overlay-play").hide()}if(d&&a.pluginType=="native"){a.load();a.play()}if(b.options.success)typeof b.options.success==
"string"?window[b.options.success](b.media,b.domNode,b):b.options.success(b.media,b.domNode,b)}},handleError:function(a){this.controls.hide();this.options.error&&this.options.error(a)},setPlayerSize:function(a,c){if(typeof a!="undefined")this.width=a;if(typeof c!="undefined")this.height=c;if(this.height.toString().indexOf("%")>0){var b=this.media.videoWidth&&this.media.videoWidth>0?this.media.videoWidth:this.options.defaultVideoWidth,e=this.media.videoHeight&&this.media.videoHeight>0?this.media.videoHeight:
this.options.defaultVideoHeight,d=this.container.parent().width();b=parseInt(d*e/b,10);if(this.container.parent()[0].tagName.toLowerCase()==="body"){d=f(window).width();b=f(window).height()}this.container.width(d).height(b);this.$media.width("100%").height("100%");this.container.find("object, embed, iframe").width("100%").height("100%");this.media.setVideoSize&&this.media.setVideoSize(d,b);this.layers.children(".mejs-layer").width("100%").height("100%")}else{this.options.overlayControls?this.container.width(this.width).height(this.height):
this.container.width(this.width).height(parseInt(this.height)+this.controls.outerHeight());this.layers.children(".mejs-layer").width(this.width).height(this.height)}},setControlsSize:function(){var a=0,c=0,b=this.controls.find(".mejs-time-rail"),e=this.controls.find(".mejs-time-total");this.controls.find(".mejs-time-current");this.controls.find(".mejs-time-loaded");var d=b.siblings();if(this.options&&!this.options.autosizeProgress)c=parseInt(b.css("width"));if(c===0||!c){d.each(function(){if(f(this).css("position")!=
"absolute")a+=f(this).outerWidth(true)});c=this.controls.width()-a-(b.outerWidth(true)-b.width())}b.width(c);e.width(c-(e.outerWidth(true)-e.width()));this.setProgressRail&&this.setProgressRail();this.setCurrentRail&&this.setCurrentRail()},buildposter:function(a,c,b,e){var d=f('<div class="mejs-poster mejs-layer"></div>').appendTo(b);c=a.$media.attr("poster");if(a.options.poster!=="")c=a.options.poster;c!==""&&c!=null?this.setPoster(c):d.hide();e.addEventListener("play",function(){d.hide()},false)},
setPoster:function(a){var c=this.container.find(".mejs-poster"),b=c.find("img");if(b.length==0)b=f('<img width="100%" height="100%" />').appendTo(c);b.attr("src",a)},buildoverlays:function(a,c,b,e){if(a.isVideo){var d=f('<div class="mejs-overlay mejs-layer"><div class="mejs-overlay-loading"><span></span></div></div>').hide().appendTo(b),g=f('<div class="mejs-overlay mejs-layer"><div class="mejs-overlay-error"></div></div>').hide().appendTo(b),j=f('<div class="mejs-overlay mejs-layer mejs-overlay-play"><div class="mejs-overlay-button"></div></div>').appendTo(b).click(function(){e.paused?
e.play():e.pause()});e.addEventListener("play",function(){j.hide();d.hide();c.find(".mejs-time-buffering").hide();g.hide()},false);e.addEventListener("playing",function(){j.hide();d.hide();c.find(".mejs-time-buffering").hide();g.hide()},false);e.addEventListener("seeking",function(){d.show();c.find(".mejs-time-buffering").show()},false);e.addEventListener("seeked",function(){d.hide();c.find(".mejs-time-buffering").hide()},false);e.addEventListener("pause",function(){mejs.MediaFeatures.isiPhone||j.show()},
false);e.addEventListener("waiting",function(){d.show();c.find(".mejs-time-buffering").show()},false);e.addEventListener("loadeddata",function(){d.show();c.find(".mejs-time-buffering").show()},false);e.addEventListener("canplay",function(){d.hide();c.find(".mejs-time-buffering").hide()},false);e.addEventListener("error",function(){d.hide();c.find(".mejs-time-buffering").hide();g.show();g.find("mejs-overlay-error").html("Error loading this resource")},false)}},buildkeyboard:function(a,c,b,e){f(document).keydown(function(d){if(a.hasFocus&&
a.options.enableKeyboard)for(var g=0,j=a.options.keyActions.length;g<j;g++)for(var i=a.options.keyActions[g],l=0,m=i.keys.length;l<m;l++)if(d.keyCode==i.keys[l]){d.preventDefault();i.action(a,e);return false}return true});f(document).click(function(d){if(f(d.target).closest(".mejs-container").length==0)a.hasFocus=false})},findTracks:function(){var a=this,c=a.$media.find("track");a.tracks=[];c.each(function(b,e){e=f(e);a.tracks.push({srclang:e.attr("srclang").toLowerCase(),src:e.attr("src"),kind:e.attr("kind"),
label:e.attr("label")||"",entries:[],isLoaded:false})})},changeSkin:function(a){this.container[0].className="mejs-container "+a;this.setPlayerSize(this.width,this.height);this.setControlsSize()},play:function(){this.media.play()},pause:function(){this.media.pause()},load:function(){this.media.load()},setMuted:function(a){this.media.setMuted(a)},setCurrentTime:function(a){this.media.setCurrentTime(a)},getCurrentTime:function(){return this.media.currentTime},setVolume:function(a){this.media.setVolume(a)},
...
'</span></button><button class="mejs-time-handle-forward visuallyhidden" aria-live="polite"><span class="visuallyhidden" aria-live="polite" aria-hidden="false">'+this.options.forwardText+'</span></button></span><span class="mejs-time-float"><span class="mejs-time-float-current">00:00</span><span class="mejs-time-float-corner"></span></span></span></div>').appendTo(c);c.find(".mejs-time-buffering").hide();var d=this,g=c.find(".mejs-time-total");b=c.find(".mejs-time-loaded");var j=c.find(".mejs-time-current"),
i=c.find(".mejs-time-handle"),l=i.find(".mejs-time-handle-rewind"),m=i.find(".mejs-time-handle-forward"),o=c.find(".mejs-time-float"),r=c.find(".mejs-time-float-current"),t=function(h){h=h.pageX;var n=g.offset(),q=g.outerWidth(),s=0;s=0;var u=h-n.left;if(h>n.left&&h<=q+n.left&&e.duration){s=(h-n.left)/q;s=s<=0.02?0:s*e.duration;k&&e.setCurrentTime(s);if(!mejs.MediaFeatures.hasTouch){o.css("left",u);r.html(mejs.Utility.secondsToTimeCode(s));o.show()}}},k=false,p=false;g.bind("mousedown",function(h){if(h.which===
1){k=true;t(h);f(document).bind("mousemove.dur",function(n){t(n)}).bind("mouseup.dur",function(){k=false;o.hide();f(document).unbind(".dur")});return false}}).bind("mouseenter",function(){p=true;f(document).bind("mousemove.dur",function(h){t(h)});mejs.MediaFeatures.hasTouch||o.show()}).bind("mouseleave",function(){p=false;if(!k){f(document).unbind(".dur");o.hide()}});i.focusin(function(){l.removeClass("visuallyhidden");m.removeClass("visuallyhidden")});i.focusout(function(){l.addClass("visuallyhidden");
m.addClass("visuallyhidden");l.children("span").text(d.options.rewindText);m.children("span").text(d.options.forwardText)});l.click(function(){if(!isNaN(e.duration)&&e.duration>0){e.setCurrentTime(Math.max(e.currentTime-e.duration*d.options.seekDistance,0));l.children("span").text(mejs.Utility.formatTimeForScreenReaders(mejs.Utility.secondsToTimeCode(d.media.currentTime),d.options.hours,d.options.minutes,d.options.seconds))}});m.click(function(){if(!isNaN(e.duration)&&e.duration>0){e.setCurrentTime(Math.min(e.currentTime+
e.duration*d.options.seekDistance,e.duration));m.children("span").text(mejs.Utility.formatTimeForScreenReaders(mejs.Utility.secondsToTimeCode(d.media.currentTime),d.options.hours,d.options.minutes,d.options.seconds))}});f(document).bind("mouseup",function(){k=false;o.hide()}).bind("mousemove",function(h){if(k||p)t(h)});e.addEventListener("progress",function(h){a.setProgressRail(h);a.setCurrentRail(h)},false);e.addEventListener("timeupdate",function(h){a.setProgressRail(h);a.setCurrentRail(h)},false);
d.loaded=b;d.total=g;d.current=j;d.handle=i},setProgressRail:function(a){var c=a!=undefined?a.target:this.media,b=null;if(c&&c.buffered&&c.buffered.length>0&&c.buffered.end&&c.duration)b=c.buffered.end(0)/c.duration;else if(c&&c.bytesTotal!=undefined&&c.bytesTotal>0&&c.bufferedBytes!=undefined)b=c.bufferedBytes/c.bytesTotal;else if(a&&a.lengthComputable&&a.total!=0)b=a.loaded/a.total;if(b!==null){b=Math.min(1,Math.max(0,b));this.loaded&&this.total&&this.loaded.width(this.total.width()*b)}},setCurrentRail:function(){if(this.media.currentTime!=
undefined&&this.media.duration)if(this.total&&this.handle){var a=this.total.width()*this.media.currentTime/this.media.duration,c=a-this.handle.outerWidth(true)/2;this.current.width(a);this.handle.css("left",c)}}})})(mejs.$);
...
e.setVolume(n)},k=false,p=false;j.bind("mouseenter",function(){i.show();r(e.volume);p=true}).bind("mouseleave",function(){p=false;k||i.hide()}).children("button").bind("focus",function(){i.show();p=true});c.click(function(){e.setVolume(Math.min(e.volume+0.1,1))});b.click(function(){e.setVolume(Math.max(e.volume-0.1,0))});f(document).on("focus","button,a",{volumeSlider:i},function(h){f(this).closest(j).length==0&&h.data.volumeSlider.hide()});i.bind("mouseover",function(){p=true}).bind("mousedown",
function(h){t(h);f(document).bind("mousemove.vol",function(n){t(n)}).bind("mouseup.vol",function(){k=false;f(document).unbind(".vol");!p&&g=="vertical"&&i.hide()});k=true;return false});j.find(":first").click(function(){e.setMuted(!e.muted)});e.addEventListener("volumechange",function(){var h=j.children("button").children("span");if(!k)if(e.muted){r(0);j.removeClass("mejs-mute").addClass("mejs-unmute");h.text(d.options.muteTextUnmute)}else{r(e.volume);j.removeClass("mejs-unmute").addClass("mejs-mute");
h.text(d.options.muteTextMute)}},false);if(d.container.is(":visible")){r(a.options.startVolume);e.pluginType==="native"&&e.setVolume(a.options.startVolume)}}}})})(mejs.$);
...
a.container;b.bind(mejs.MediaFeatures.fullScreenEventName,function(){if(mejs.MediaFeatures.isFullScreen()){a.isNativeFullScreen=true;a.setControlsSize()}else{a.isNativeFullScreen=false;a.exitFullScreen()}})}var d=this,g=f('<div class="mejs-button mejs-fullscreen-button"><button type="button" aria-controls="'+d.id+'" aria-live="polite"><span class="visuallyhidden">'+d.options.fullscreenText+"</span></button></div>").appendTo(c);if(d.media.pluginType==="native"||!d.options.usePluginFullScreen&&!mejs.MediaFeatures.isFirefox)g.click(function(){mejs.MediaFeatures.hasTrueNativeFullScreen&&
mejs.MediaFeatures.isFullScreen()||a.isFullScreen?a.exitFullScreen():a.enterFullScreen()});else{var j=null;c=function(){var k=document.createElement("x"),p=document.documentElement,h=window.getComputedStyle;if(!("pointerEvents"in k.style))return false;k.style.pointerEvents="auto";k.style.pointerEvents="x";p.appendChild(k);h=h&&h(k,"").pointerEvents==="auto";p.removeChild(k);return!!h}();console.log("supportsPointerEvents",c);if(c&&!mejs.MediaFeatures.isOpera){var i=false,l=function(){if(i){m.hide();
o.hide();r.hide();g.css("pointer-events","");d.controls.css("pointer-events","");i=false}},m=f('<div class="mejs-fullscreen-hover" />').appendTo(d.container).mouseover(l),o=f('<div class="mejs-fullscreen-hover"  />').appendTo(d.container).mouseover(l),r=f('<div class="mejs-fullscreen-hover"  />').appendTo(d.container).mouseover(l),t=function(){var k={position:"absolute",top:0,left:0};m.css(k);o.css(k);r.css(k);m.width(d.container.width()).height(d.container.height()-d.controls.height());k=g.offset().left-
d.container.offset().left;fullScreenBtnWidth=g.outerWidth(true);o.width(k).height(d.controls.height()).css({top:d.container.height()-d.controls.height()});r.width(d.container.width()-k-fullScreenBtnWidth).height(d.controls.height()).css({top:d.container.height()-d.controls.height(),left:k+fullScreenBtnWidth})};f(document).resize(function(){t()});g.mouseover(function(){if(!d.isFullScreen){var k=g.offset(),p=a.container.offset();e.positionFullscreenButton(k.left-p.left,k.top-p.top,false);g.css("pointer-events",
"none");d.controls.css("pointer-events","none");m.show();r.show();o.show();t();i=true}});e.addEventListener("fullscreenchange",function(){l()})}else g.mouseover(function(){if(j!==null){clearTimeout(j);delete j}var k=g.offset(),p=a.container.offset();e.positionFullscreenButton(k.left-p.left,k.top-p.top,true)}).mouseout(function(){if(j!==null){clearTimeout(j);delete j}j=setTimeout(function(){e.hideFullscreenButton()},1500)})}a.fullscreenBtn=g;f(document).bind("keydown",function(k){if((mejs.MediaFeatures.hasTrueNativeFullScreen&&
mejs.MediaFeatures.isFullScreen()||d.isFullScreen)&&k.keyCode==27)a.exitFullScreen()})}},enterFullScreen:function(){var a=this;if(!(a.media.pluginType!=="native"&&(mejs.MediaFeatures.isFirefox||a.options.usePluginFullScreen))){docStyleOverflow=document.documentElement.style.overflow;document.documentElement.style.overflow="hidden";normalHeight=a.container.height();normalWidth=a.container.width();if(a.media.pluginType==="native")if(mejs.MediaFeatures.hasTrueNativeFullScreen){mejs.MediaFeatures.requestFullScreen(a.container[0]);
a.isInIframe&&setTimeout(function b(){if(a.isNativeFullScreen)f(window).width()!==screen.width?a.exitFullScreen():setTimeout(b,500)},500)}else if(mejs.MediaFeatures.hasSemiNativeFullScreen){a.media.webkitEnterFullscreen();return}if(a.isInIframe){var c=a.options.newWindowCallback(this);if(c!=="")if(mejs.MediaFeatures.hasTrueNativeFullScreen)setTimeout(function(){if(!a.isNativeFullScreen){a.pause();window.open(c,a.id,"top=0,left=0,width="+screen.availWidth+",height="+screen.availHeight+",resizable=yes,scrollbars=no,status=no,toolbar=no")}},
...
lang){a.selectedTrack=a.tracks[g];a.captions.attr("lang",a.selectedTrack.srclang);a.displayCaptions();break}});if(!a.options.alwaysShowControls&&a.options.overlayControls){var j;a.container.bind("mouseenter mouseover focusin",function(){clearTimeout(j);j=null;a.container.find(".mejs-captions-position").addClass("mejs-captions-position-hover")}).bind("mouseleave focusout",function(){e.paused||j||(j=setTimeout(function(){a.container.find(".mejs-captions-position").removeClass("mejs-captions-position-hover")},
