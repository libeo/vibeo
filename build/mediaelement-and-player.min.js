...
function(h){t(h);f(document).bind("mousemove.vol",function(n){t(n)}).bind("mouseup.vol",function(){k=false;f(document).unbind(".vol");!p&&g=="vertical"&&i.hide()});k=true;return false});j.find(":first").click(function(){e.setMuted(!e.muted)}).focus(function(){f(this).attr("aria-live","polite")}).blur(function(){f(this).attr("aria-live","off")});e.addEventListener("volumechange",function(){var h=j.children("button").children("span");if(!k)if(e.muted){r(0);j.removeClass("mejs-mute").addClass("mejs-unmute");
h.text(d.options.muteTextUnmute)}else{r(e.volume);j.removeClass("mejs-unmute").addClass("mejs-mute");h.text(d.options.muteTextMute)}},false);if(d.container.is(":visible")){r(a.options.startVolume);e.pluginType==="native"&&e.setVolume(a.options.startVolume)}}}})})(mejs.$);
...
a.container;b.bind(mejs.MediaFeatures.fullScreenEventName,function(){if(mejs.MediaFeatures.isFullScreen()){a.isNativeFullScreen=true;a.setControlsSize()}else{a.isNativeFullScreen=false;a.exitFullScreen()}})}var d=this,g=f('<div class="mejs-button mejs-fullscreen-button"><button type="button" aria-controls="'+d.id+'" aria-live="polite"><span class="visuallyhidden">'+d.options.fullscreenText+"</span></button></div>").appendTo(c);f("#"+d.id).parent().on("focus","button.guard",function(){g.trigger("click")});
if(d.media.pluginType==="native"||!d.options.usePluginFullScreen&&!mejs.MediaFeatures.isFirefox)g.click(function(){if(mejs.MediaFeatures.hasTrueNativeFullScreen&&mejs.MediaFeatures.isFullScreen()||a.isFullScreen){a.exitFullScreen();f("#"+d.id).parent().find("button.guard").remove()}else{a.enterFullScreen();f("#"+d.id).before('<button class="guard visuallyhidden">Quitter le mode plein \u00e9cran</button>');f("#"+d.id).after('<button class="guard visuallyhidden">Quitter le mode plein \u00e9cran</button>')}});
else{var j=null;c=function(){var k=document.createElement("x"),p=document.documentElement,h=window.getComputedStyle;if(!("pointerEvents"in k.style))return false;k.style.pointerEvents="auto";k.style.pointerEvents="x";p.appendChild(k);h=h&&h(k,"").pointerEvents==="auto";p.removeChild(k);return!!h}();console.log("supportsPointerEvents",c);if(c&&!mejs.MediaFeatures.isOpera){var i=false,l=function(){if(i){m.hide();o.hide();r.hide();g.css("pointer-events","");d.controls.css("pointer-events","");i=false}},
m=f('<div class="mejs-fullscreen-hover" />').appendTo(d.container).mouseover(l),o=f('<div class="mejs-fullscreen-hover"  />').appendTo(d.container).mouseover(l),r=f('<div class="mejs-fullscreen-hover"  />').appendTo(d.container).mouseover(l),t=function(){var k={position:"absolute",top:0,left:0};m.css(k);o.css(k);r.css(k);m.width(d.container.width()).height(d.container.height()-d.controls.height());k=g.offset().left-d.container.offset().left;fullScreenBtnWidth=g.outerWidth(true);o.width(k).height(d.controls.height()).css({top:d.container.height()-
d.controls.height()});r.width(d.container.width()-k-fullScreenBtnWidth).height(d.controls.height()).css({top:d.container.height()-d.controls.height(),left:k+fullScreenBtnWidth})};f(document).resize(function(){t()});g.mouseover(function(){if(!d.isFullScreen){var k=g.offset(),p=a.container.offset();e.positionFullscreenButton(k.left-p.left,k.top-p.top,false);g.css("pointer-events","none");d.controls.css("pointer-events","none");m.show();r.show();o.show();t();i=true}});e.addEventListener("fullscreenchange",
function(){l()})}else g.mouseover(function(){if(j!==null){clearTimeout(j);delete j}var k=g.offset(),p=a.container.offset();e.positionFullscreenButton(k.left-p.left,k.top-p.top,true)}).mouseout(function(){if(j!==null){clearTimeout(j);delete j}j=setTimeout(function(){e.hideFullscreenButton()},1500)})}a.fullscreenBtn=g;f(document).bind("keydown",function(k){if((mejs.MediaFeatures.hasTrueNativeFullScreen&&mejs.MediaFeatures.isFullScreen()||d.isFullScreen)&&k.keyCode==27)a.exitFullScreen()})}},enterFullScreen:function(){var a=
this;if(!(a.media.pluginType!=="native"&&(mejs.MediaFeatures.isFirefox||a.options.usePluginFullScreen))){docStyleOverflow=document.documentElement.style.overflow;document.documentElement.style.overflow="hidden";normalHeight=a.container.height();normalWidth=a.container.width();if(a.media.pluginType==="native")if(mejs.MediaFeatures.hasTrueNativeFullScreen){mejs.MediaFeatures.requestFullScreen(a.container[0]);a.isInIframe&&setTimeout(function b(){if(a.isNativeFullScreen)f(window).width()!==screen.width?
a.exitFullScreen():setTimeout(b,500)},500)}else if(mejs.MediaFeatures.hasSemiNativeFullScreen){a.media.webkitEnterFullscreen();return}if(a.isInIframe){var c=a.options.newWindowCallback(this);if(c!=="")if(mejs.MediaFeatures.hasTrueNativeFullScreen)setTimeout(function(){if(!a.isNativeFullScreen){a.pause();window.open(c,a.id,"top=0,left=0,width="+screen.availWidth+",height="+screen.availHeight+",resizable=yes,scrollbars=no,status=no,toolbar=no")}},250);else{a.pause();window.open(c,a.id,"top=0,left=0,width="+
...
docStyleOverflow;this.container.removeClass("mejs-container-fullscreen").width(normalWidth).height(normalHeight);if(this.pluginType==="native")this.$media.width(normalWidth).height(normalHeight);else if(this.options.overlayControls){this.container.find("object embed").width(normalWidth).height(normalHeight);this.media.setVideoSize(normalWidth,normalHeight)}else this.$media.width(normalWidth).height(parseInt(normalHeight)-this.controls.outerHeight());this.layers.children("div").width(normalWidth).height(normalHeight);
