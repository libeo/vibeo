...
10);var e=parseInt(a[1],10),g=parseInt(a[2],10),f=0,h=0;if(c)f=parseInt(a[3])/d;return h=b*3600+e*60+g+f},formatTimeForScreenReaders:function(a,b,c,d){a=a.split(":");var e="";if(a.length>2){if(a[0]!="00")e+=a[0]+" "+b;if(a[1]!="00")e+=a[1]+" "+c;e+=a[2]+" "+d}else{if(a.length>2&&a[0]!="00")e+=a[0]+" "+c;e+=a[1]+" "+d}return e},convertSMPTEtoSeconds:function(a){if(typeof a!="string")return false;a=a.replace(",",".");var b=0,c=a.indexOf(".")!=-1?a.split(".")[1].length:0,d=1;a=a.split(":").reverse();
for(var e=0;e<a.length;e++){d=1;if(e>0)d=Math.pow(60,e);b+=Number(a[e])*d}return Number(b.toFixed(c))},removeSwf:function(a){var b=document.getElementById(a);if(b&&b.nodeName=="OBJECT")if(mejs.MediaFeatures.isIE){b.style.display="none";(function(){b.readyState==4?mejs.Utility.removeObjectInIE(a):setTimeout(arguments.callee,10)})()}else b.parentNode.removeChild(b)},removeObjectInIE:function(a){if(a=document.getElementById(a)){for(var b in a)if(typeof a[b]=="function")a[b]=null;a.parentNode.removeChild(a)}}};
...
alwaysShowControls:false,overlayControls:true,clickToPlayPause:true,iPadUseNativeControls:false,iPhoneUseNativeControls:false,AndroidUseNativeControls:false,tooltip:"Help",features:["playpause","current","progress","duration","tracks","volume","fullscreen"],isVideo:true,enableKeyboard:true,pauseOtherPlayers:true,keyActions:[{keys:[179],action:function(a,b){b.paused||b.ended?b.play():b.pause()}},{keys:[38],action:function(a,b){b.setVolume(Math.min(b.volume+0.1,1))}},{keys:[40],action:function(a,b){b.setVolume(Math.max(b.volume-
0.1,0))}},{keys:[37,227],action:function(a,b){if(!isNaN(b.duration)&&b.duration>0){a.isVideo&&a.showControls();var c=Math.max(b.currentTime-a.options.defaultSeekBackwardInterval(b),0);b.setCurrentTime(c);f(".mejs-time-temp").text(mejs.Utility.formatTimeForScreenReaders(mejs.Utility.secondsToTimeCode(c),a.options.hours,a.options.minutes,a.options.seconds))}}},{keys:[39,228],action:function(a,b){if(!isNaN(b.duration)&&b.duration>0){if(a.isVideo){a.showControls();a.startControlsTimer()}var c=Math.min(b.currentTime+
a.options.defaultSeekForwardInterval(b),b.duration);b.setCurrentTime(c);f(".mejs-time-temp").text(mejs.Utility.formatTimeForScreenReaders(mejs.Utility.secondsToTimeCode(c),a.options.hours,a.options.minutes,a.options.seconds))}}},{keys:[70],action:function(a){if(typeof a.enterFullScreen!="undefined")a.isFullScreen?a.exitFullScreen():a.enterFullScreen()}}]};mejs.mepIndex=0;mejs.players=[];mejs.MediaElementPlayer=function(a,b){if(!(this instanceof mejs.MediaElementPlayer))return new mejs.MediaElementPlayer(a,
b);this.$media=this.$node=f(a);this.node=this.media=this.$media[0];if(typeof this.node.player!="undefined")return this.node.player;else this.node.player=this;if(typeof b=="undefined")b=this.$node.data("mejsoptions");this.options=f.extend({},mejs.MepDefaults,b);mejs.players.push(this);this.init();return this};mejs.MediaElementPlayer.prototype={hasFocus:false,controlsAreVisible:true,init:function(){var a=this,b=mejs.MediaFeatures,c=f.extend(true,{},a.options,{success:function(d,g){a.meReady(d,g)},error:function(d){a.handleError(d)}}),
e=a.media.tagName.toLowerCase();a.isDynamic=e!=="audio"&&e!=="video";a.isVideo=a.isDynamic?a.options.isVideo:e!=="audio"&&a.options.isVideo;if(b.isiPad&&a.options.iPadUseNativeControls||b.isiPhone&&a.options.iPhoneUseNativeControls){a.$media.attr("controls","controls");if(b.isiPad&&a.media.getAttribute("autoplay")!==null){a.media.load();a.media.play()}}else if(!(b.isAndroid&&a.AndroidUseNativeControls)){a.$media.removeAttr("controls");a.id="mep_"+mejs.mepIndex++;a.container=f('<div id="'+a.id+'" class="mejs-container '+
(mejs.MediaFeatures.svg?"svg":"no-svg")+'"><div class="mejs-inner"><div class="mejs-mediaelement"></div><div class="mejs-layers"></div><div class="mejs-controls"></div><div class="mejs-clear"></div></div></div>').addClass(a.$media[0].className).insertBefore(a.$media);a.container.addClass((b.isAndroid?"mejs-android ":"")+(b.isiOS?"mejs-ios ":"")+(b.isiPad?"mejs-ipad ":"")+(b.isiPhone?"mejs-iphone ":"")+(a.isVideo?"mejs-video ":"mejs-audio "));if(b.isiOS){b=a.$media.clone();a.container.find(".mejs-mediaelement").append(b);
a.$media.remove();a.$node=a.$media=b;a.node=a.media=b[0]}else a.container.find(".mejs-mediaelement").append(a.$media);setTimeout(function(){var d=a.container.find("video");d.attr("tabindex")==0&&d.removeAttr("tabindex")},200);a.controls=a.container.find(".mejs-controls");a.layers=a.container.find(".mejs-layers");b=a.isVideo?"video":"audio";e=b.substring(0,1).toUpperCase()+b.substring(1);a.width=a.options[b+"Width"]>0||a.options[b+"Width"].toString().indexOf("%")>-1?a.options[b+"Width"]:a.media.style.width!==
""&&a.media.style.width!==null?a.media.style.width:a.media.getAttribute("width")!==null?a.$media.attr("width"):a.options["default"+e+"Width"];a.height=a.options[b+"Height"]>0||a.options[b+"Height"].toString().indexOf("%")>-1?a.options[b+"Height"]:a.media.style.height!==""&&a.media.style.height!==null?a.media.style.height:a.$media[0].getAttribute("height")!==null?a.$media.attr("height"):a.options["default"+e+"Height"];a.setPlayerSize(a.width,a.height);c.pluginWidth=a.height;c.pluginHeight=a.width}popuptooltip=
function(){f(".bubble-text").show("fast");f(".bubble-text").css("display","block")};hidetooltip=function(){f(".bubble-text").hide("fast")};f(".mejs-tooltip").mouseenter(popuptooltip);f(".mejs-tooltip").focusin(popuptooltip);f(".mejs-tooltip").mouseleave(hidetooltip);f(".mejs-tooltip").focusout(hidetooltip);mejs.MediaElement(a.$media[0],c);a.container.trigger("controlsshown")},showControls:function(a){var b=this;a=typeof a=="undefined"||a;if(!b.controlsAreVisible){if(a){b.controls.css("visibility",
"visible").stop(true,true).fadeIn(200,function(){b.controlsAreVisible=true;b.container.trigger("controlsshown")});b.container.find(".mejs-control").css("visibility","visible").stop(true,true).fadeIn(200,function(){b.controlsAreVisible=true})}else{b.controls.css("visibility","visible").css("display","block");b.container.find(".mejs-control").css("visibility","visible").css("display","block");b.controlsAreVisible=true;b.container.trigger("controlsshown")}b.setControlsSize()}},hideControls:function(a){var b=
this;a=typeof a=="undefined"||a;if(b.controlsAreVisible)if(a){b.controls.stop(true,true).fadeOut(200,function(){f(this).css("visibility","hidden").css("display","block");b.controlsAreVisible=false;b.container.trigger("controlshidden")});b.container.find(".mejs-control").stop(true,true).fadeOut(200,function(){f(this).css("visibility","hidden").css("display","block")})}else{b.controls.css("visibility","hidden").css("display","block");b.container.find(".mejs-control").css("visibility","hidden").css("display",
"block");b.controlsAreVisible=false;b.container.trigger("controlshidden")}},controlsTimer:null,startControlsTimer:function(a){var b=this;a=typeof a!="undefined"?a:1500;b.killControlsTimer("start");b.controlsTimer=setTimeout(function(){b.hideControls();b.killControlsTimer("hide")},a)},killControlsTimer:function(){if(this.controlsTimer!==null){clearTimeout(this.controlsTimer);delete this.controlsTimer;this.controlsTimer=null}},controlsEnabled:true,disableControls:function(){this.killControlsTimer();
...
function(){c.controlsEnabled&&!c.media.paused&&!c.options.alwaysShowControls&&c.startControlsTimer(1E3)})}d&&!c.options.alwaysShowControls&&c.hideControls();c.options.enableAutosize&&c.media.addEventListener("loadedmetadata",function(i){if(c.options.videoHeight<=0&&c.domNode.getAttribute("height")===null&&!isNaN(i.target.videoHeight)){c.setPlayerSize(i.target.videoWidth,i.target.videoHeight);c.setControlsSize();c.media.setVideoSize(i.target.videoWidth,i.target.videoHeight)}},false)}a.addEventListener("play",
function(){for(var i=0,l=mejs.players.length;i<l;i++){var m=mejs.players[i];m.id!=c.id&&c.options.pauseOtherPlayers&&!m.paused&&!m.ended&&m.pause();m.hasFocus=false}c.hasFocus=true},false);a.addEventListener("focus",function(){for(var i=0,l=mejs.players.length;i<l;i++){var m=mejs.players[i];m.id!=c.id&&c.options.pauseOtherPlayers&&!m.paused&&!m.ended&&m.pause();m.hasFocus=false}c.hasFocus=true});c.media.addEventListener("ended",function(){if(c.options.autoRewind)try{c.media.setCurrentTime(0)}catch(i){}c.media.pause();
...
this.options.defaultAudioHeight,d=this.container.parent().closest(":visible").width();c=this.isVideo||!this.options.autosizeProgress?parseInt(d*e/c,10):e;if(this.container.parent()[0].tagName.toLowerCase()==="body"){d=f(window).width();c=f(window).height()}if(c!=0&&d!=0){this.container.width(d).height(c);this.$media.width("100%").height("100%");this.container.find("object, embed, iframe").width("100%").height("100%");this.isVideo&&this.media.setVideoSize&&this.media.setVideoSize(d,c);this.layers.children(".mejs-layer").width("100%").height("100%")}}else{this.container.width(this.width).height(this.height);
...
0)c=f('<img width="100%" height="100%" />').appendTo(b);c.attr("src",a)},buildoverlays:function(a,b,c,e){var d=this;if(a.isVideo){var g=f('<div class="mejs-overlay mejs-layer"><div class="mejs-overlay-loading"><span></span></div></div>').hide().appendTo(c),j=f('<div class="mejs-overlay mejs-layer"><div class="mejs-overlay-error"></div></div>').hide().appendTo(c),i=f('<div class="mejs-overlay mejs-layer mejs-overlay-play"><div class="mejs-overlay-button"></div></div>').appendTo(c).click(function(){if(d.options.clickToPlayPause)e.paused?
e.play():e.pause()});e.addEventListener("play",function(){i.hide();g.hide();b.find(".mejs-time-buffering").hide();j.hide()},false);e.addEventListener("playing",function(){i.hide();g.hide();b.find(".mejs-time-buffering").hide();j.hide()},false);e.addEventListener("seeking",function(){g.show();b.find(".mejs-time-buffering").show()},false);e.addEventListener("seeked",function(){g.hide();b.find(".mejs-time-buffering").hide()},false);e.addEventListener("pause",function(){mejs.MediaFeatures.isiPhone||i.show()},
false);e.addEventListener("waiting",function(){g.show();b.find(".mejs-time-buffering").show()},false);e.addEventListener("loadeddata",function(){g.show();b.find(".mejs-time-buffering").show()},false);e.addEventListener("canplay",function(){g.hide();b.find(".mejs-time-buffering").hide()},false);e.addEventListener("error",function(){g.hide();b.find(".mejs-time-buffering").hide();j.show();j.find("mejs-overlay-error").html("Error loading this resource")},false)}},buildkeyboard:function(a,b,c,e){f(document).keydown(function(d){if(a.hasFocus&&
a.options.enableKeyboard)for(var g=0,j=a.options.keyActions.length;g<j;g++)for(var i=a.options.keyActions[g],l=0,m=i.keys.length;l<m;l++)if(d.keyCode==i.keys[l]){d.preventDefault();i.action(a,e,d.keyCode);return false}return true});f(document).click(function(d){if(f(d.target).closest(".mejs-container").length==0)a.hasFocus=false})},findTracks:function(){var a=this,b=a.$media.find("track");a.tracks=[];b.each(function(c,e){e=f(e);a.tracks.push({srclang:e.attr("srclang").toLowerCase(),src:e.attr("src"),
...
'</span></button><button class="mejs-time-handle-forward visuallyhidden" aria-live="polite"><span class="visuallyhidden" aria-live="polite" aria-hidden="false">'+this.options.forwardText+'</span></button></span><span class="mejs-time-float"><span class="mejs-time-float-current">00:00</span><span class="mejs-time-float-corner"></span></span></span></div>').appendTo(b);b.find(".mejs-time-buffering").hide();var d=this,g=b.find(".mejs-time-total");c=b.find(".mejs-time-loaded");var j=b.find(".mejs-time-current"),
i=b.find(".mejs-time-handle"),l=i.find(".mejs-time-handle-rewind"),m=i.find(".mejs-time-handle-forward"),o=b.find(".mejs-time-float"),s=b.find(".mejs-time-float-current"),t=function(h){h=h.pageX;var n=g.offset(),q=g.outerWidth(true),r=0,u=r=0;if(e.duration){if(h<n.left)h=n.left;else if(h>q+n.left)h=q+n.left;u=h-n.left;r=u/q;r=r<=0.02?0:r*e.duration;k&&r!==e.currentTime&&e.setCurrentTime(r);if(!mejs.MediaFeatures.hasTouch){o.css("left",u);s.html(mejs.Utility.secondsToTimeCode(r));o.show()}}},k=false,
p=false;g.bind("mousedown",function(h){if(h.which===1){k=true;t(h);f(document).bind("mousemove.dur",function(n){t(n)}).bind("mouseup.dur",function(){k=false;o.hide();f(document).unbind(".dur")});return false}}).bind("mouseenter",function(){p=true;f(document).bind("mousemove.dur",function(h){t(h)});mejs.MediaFeatures.hasTouch||o.show()}).bind("mouseleave",function(){p=false;if(!k){f(document).unbind(".dur");o.hide()}});i.focusin(function(){l.removeClass("visuallyhidden");m.removeClass("visuallyhidden")});
i.focusout(function(){l.addClass("visuallyhidden");m.addClass("visuallyhidden");l.children("span").text(d.options.rewindText);m.children("span").text(d.options.forwardText)});l.click(function(){if(!isNaN(e.duration)&&e.duration>0){e.setCurrentTime(Math.max(e.currentTime-e.duration*d.options.seekDistance,0));l.children("span").text(mejs.Utility.formatTimeForScreenReaders(mejs.Utility.secondsToTimeCode(d.media.currentTime),d.options.hours,d.options.minutes,d.options.seconds)+" de "+mejs.Utility.formatTimeForScreenReaders(mejs.Utility.secondsToTimeCode(d.media.duration),
...
f(document).bind("mouseup",function(){k=false;o.hide()}).bind("mousemove",function(h){if(k||p)t(h)});e.addEventListener("progress",function(h){a.setProgressRail(h);a.setCurrentRail(h)},false);e.addEventListener("timeupdate",function(h){a.setProgressRail(h);a.setCurrentRail(h)},false);d.loaded=c;d.total=g;d.current=j;d.handle=i},setProgressRail:function(a){var b=a!=undefined?a.target:this.media,c=null;if(b&&b.buffered&&b.buffered.length>0&&b.buffered.end&&b.duration)c=b.buffered.end(0)/b.duration;
...
d.options.volumeUpText+"</span></button></div></div>").appendTo(b),i=d.container.find(".mejs-volume-slider"),l=d.container.find(".mejs-volume-total"),m=d.container.find(".mejs-volume-current"),o=d.container.find(".mejs-volume-handle");b=d.container.find(".mejs-volume-plus");c=d.container.find(".mejs-volume-minus");var s=function(h){var n=i.is(":visible");!n&&typeof secondTry=="undefined"&&i.show();h=Math.max(0,h);h=Math.min(h,1);h==0?j.removeClass("mejs-mute").addClass("mejs-unmute"):j.removeClass("mejs-unmute").addClass("mejs-mute");
if(g=="vertical"){var q=l.height(),r=l.position();h=q-q*h;o.css("top",Math.round(r.top+h-o.height()/2));m.height(q-h);m.css("top",r.top+h)}else{q=l.width();r=l.position();h=q*h;o.css("left",Math.round(r.left+h-o.width()/2));m.width(Math.round(h))}n||i.hide()},t=function(h){var n=null,q=l.offset();if(g=="vertical"){n=l.height();parseInt(l.css("top").replace(/px/,""),10);n=(n-(h.pageY-q.top))/n;if(q.top==0||q.left==0)return}else{n=l.width();n=(h.pageX-q.left)/n}n=Math.max(0,n);n=Math.min(n,1);s(n);
n==0?e.setMuted(true):e.setMuted(false);e.setVolume(n)},k=false,p=false;j.bind("mouseenter",function(){i.show();s(e.volume);p=true}).bind("mouseleave",function(){p=false;k||i.hide()}).children("button").bind("focus",function(){i.show();p=true});b.click(function(){e.setVolume(Math.min(e.volume+0.1,1))});c.click(function(){e.setVolume(Math.max(e.volume-0.1,0))});f(document).on("focus","button,a",{volumeSlider:i},function(h){f(this).closest(j).length==0&&h.data.volumeSlider.hide()});i.bind("mouseover",
function(){p=true}).bind("mousedown",function(h){console.log(h.offsetX);if(h.offsetX!=0){t(h);f(document).bind("mousemove.vol",function(n){t(n)}).bind("mouseup.vol",function(){k=false;f(document).unbind(".vol");!p&&g=="vertical"&&i.hide()});k=true;return false}});j.find(":first").click(function(){e.setMuted(!e.muted)}).focus(function(){f(this).attr("aria-live","polite")}).blur(function(){f(this).attr("aria-live","off")});e.addEventListener("volumechange",function(){var h=j.children("button").children("span");
if(!k)if(e.muted){s(0);j.removeClass("mejs-mute").addClass("mejs-unmute");h.text(d.options.muteTextUnmute)}else{s(e.volume);j.removeClass("mejs-unmute").addClass("mejs-mute");h.text(d.options.muteTextMute)}},false);if(d.container.is(":visible")){s(a.options.startVolume);e.pluginType==="native"&&e.setVolume(a.options.startVolume)}}}})})(mejs.$);
(function(f){f.extend(mejs.MepDefaults,{usePluginFullScreen:true,newWindowCallback:function(){return""},fullscreenText:mejs.i18n.t("Fullscreen"),unFullscreenText:"Exit FullScreen"});f.extend(MediaElementPlayer.prototype,{isFullScreen:false,isNativeFullScreen:false,docStyleOverflow:null,isInIframe:false,buildfullscreen:function(a,b,c,e){if(a.isVideo){a.isInIframe=window.location!=window.parent.location;if(mejs.MediaFeatures.hasTrueNativeFullScreen){c=null;c=mejs.MediaFeatures.hasMozNativeFullScreen?
f(document):a.container;c.bind(mejs.MediaFeatures.fullScreenEventName,function(){if(mejs.MediaFeatures.isFullScreen()){a.isNativeFullScreen=true;a.setControlsSize()}else{d.container.parent().find("button.guard").length>0&&d.container.parent().find("button.guard").remove();a.isNativeFullScreen=false;a.exitFullScreen()}})}var d=this,g=f('<div class="mejs-button mejs-fullscreen-button"><button type="button" aria-controls="'+d.id+'" aria-live="polite"><span class="visuallyhidden">'+d.options.fullscreenText+
"</span></button></div>").appendTo(b);d.container.parent().on("focus","button.guard",function(){(mejs.MediaFeatures.hasTrueNativeFullScreen&&mejs.MediaFeatures.isFullScreen()||a.isFullScreen)&&g.trigger("click")});if(d.media.pluginType==="native"||!d.options.usePluginFullScreen&&!mejs.MediaFeatures.isFirefox)g.click(function(){var k=mejs.MediaFeatures.hasTrueNativeFullScreen&&mejs.MediaFeatures.isFullScreen()||a.isFullScreen;d.container.find("#btnplaypause").focus();if(k){a.exitFullScreen();d.container.parent().find("button.guard").remove()}else{a.enterFullScreen();
d.container.before('<button class="guard visuallyhidden">Quitter le mode plein \u00e9cran</button>');d.container.after('<button class="guard visuallyhidden">Quitter le mode plein \u00e9cran</button>')}});else{var j=null;if(function(){var k=document.createElement("x"),p=document.documentElement,h=window.getComputedStyle;if(!("pointerEvents"in k.style))return false;k.style.pointerEvents="auto";k.style.pointerEvents="x";p.appendChild(k);h=h&&h(k,"").pointerEvents==="auto";p.removeChild(k);return!!h}()&&
!mejs.MediaFeatures.isOpera){var i=false,l=function(){if(i){m.hide();o.hide();s.hide();g.css("pointer-events","");d.controls.css("pointer-events","");i=false}},m=f('<div class="mejs-fullscreen-hover" />').appendTo(d.container).mouseover(l),o=f('<div class="mejs-fullscreen-hover"  />').appendTo(d.container).mouseover(l),s=f('<div class="mejs-fullscreen-hover"  />').appendTo(d.container).mouseover(l),t=function(){var k={position:"absolute",top:0,left:0};m.css(k);o.css(k);s.css(k);m.width(d.container.width()).height(d.container.height()-
d.controls.height());k=g.offset().left-d.container.offset().left;fullScreenBtnWidth=g.outerWidth(true);o.width(k).height(d.controls.height()).css({top:d.container.height()-d.controls.height()});s.width(d.container.width()-k-fullScreenBtnWidth).height(d.controls.height()).css({top:d.container.height()-d.controls.height(),left:k+fullScreenBtnWidth})};f(document).resize(function(){t()});g.mouseover(function(){if(!d.isFullScreen){var k=g.offset(),p=a.container.offset();e.positionFullscreenButton(k.left-
p.left,k.top-p.top,false);g.css("pointer-events","none");d.controls.css("pointer-events","none");m.show();s.show();o.show();t();i=true}});e.addEventListener("fullscreenchange",function(){l()})}else g.mouseover(function(){if(j!==null){clearTimeout(j);delete j}var k=g.offset(),p=a.container.offset();e.positionFullscreenButton(k.left-p.left,k.top-p.top,true)}).mouseout(function(){if(j!==null){clearTimeout(j);delete j}j=setTimeout(function(){e.hideFullscreenButton()},1500)})}a.fullscreenBtn=g;f(document).bind("keydown",
function(k){if((mejs.MediaFeatures.hasTrueNativeFullScreen&&mejs.MediaFeatures.isFullScreen()||d.isFullScreen)&&k.keyCode==27)a.exitFullScreen()})}},enterFullScreen:function(){var a=this;if(!(a.media.pluginType!=="native"&&(mejs.MediaFeatures.isFirefox||a.options.usePluginFullScreen))){docStyleOverflow=document.documentElement.style.overflow;document.documentElement.style.overflow="hidden";normalHeight=a.container.height();normalWidth=a.container.width();if(a.media.pluginType==="native")if(mejs.MediaFeatures.hasTrueNativeFullScreen){mejs.MediaFeatures.requestFullScreen(a.container[0]);
...
(function(f){f.extend(mejs.MepDefaults,{startLanguage:"",tracksTextTurnOn:"Captions/Subtitles On",tracksTextTurnOff:"Captions/Subtitles Off"});f.extend(MediaElementPlayer.prototype,{hasChapters:false,buildtracks:function(a,b,c,e){if(a.isVideo)if(a.tracks.length!=0){var d=this,g;a.chapters=f('<div class="mejs-chapters mejs-layer"></div>').prependTo(c).hide();a.captions=f('<div class="mejs-captions-layer mejs-layer"><div class="mejs-captions-position"><span class="mejs-captions-text"></span></div></div>').prependTo(c).hide();
...
lang){a.selectedTrack=a.tracks[g];a.captions.attr("lang",a.selectedTrack.srclang);a.displayCaptions();break}});if(!a.options.alwaysShowControls&&a.options.overlayControls){var j;a.container.bind("controlsshown",function(){clearTimeout(j);j=null;a.container.find(".mejs-captions-position").addClass("mejs-captions-position-hover")}).bind("controlshidden",function(){e.paused||j||(j=setTimeout(function(){a.container.find(".mejs-captions-position").removeClass("mejs-captions-position-hover")},1300))})}else a.options.overlayControls&&
...
j;g++)if(d[g].isSeparator)e+='<div class="mejs-contextmenu-separator"></div>';else{var i=d[g].render(c);if(i!=null)e+='<div class="mejs-contextmenu-item" data-itemindex="'+g+'" id="element-'+Math.random()*1E6+'">'+i+"</div>"}c.contextMenu.empty().append(f(e)).css({top:b,left:a}).show();c.contextMenu.find(".mejs-contextmenu-item").each(function(){var l=f(this),m=parseInt(l.data("itemindex"),10),o=c.options.contextMenuItems[m];typeof o.show!="undefined"&&o.show(l,c);l.click(function(){typeof o.click!=
