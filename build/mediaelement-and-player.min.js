...
true;a.setControlsSize()}else{a.isNativeFullScreen=false;a.exitFullScreen()}});var e=this,g=f('<div class="mejs-button mejs-fullscreen-button"><button type="button" aria-controls="'+e.id+'"><span class="visuallyhidden">'+e.options.fullscreenText+"</span></button></div>").appendTo(c);if(e.media.pluginType==="native"||!e.options.usePluginFullScreen&&!mejs.MediaFeatures.isFirefox)g.click(function(){mejs.MediaFeatures.hasTrueNativeFullScreen&&mejs.MediaFeatures.isFullScreen()||a.isFullScreen?a.exitFullScreen():
a.enterFullScreen()});else{var k=null;if(document.documentElement.style.pointerEvents===""&&!mejs.MediaFeatures.isOpera){var i=false,m=function(){if(i){j.hide();o.hide();q.hide();g.css("pointer-events","");e.controls.css("pointer-events","");i=false}},j=f('<div class="mejs-fullscreen-hover" />').appendTo(e.container).mouseover(m),o=f('<div class="mejs-fullscreen-hover"  />').appendTo(e.container).mouseover(m),q=f('<div class="mejs-fullscreen-hover"  />').appendTo(e.container).mouseover(m),l=function(){var h=
{position:"absolute",top:0,left:0};j.css(h);o.css(h);q.css(h);j.width(e.container.width()).height(e.container.height()-e.controls.height());h=g.offset().left-e.container.offset().left;fullScreenBtnWidth=g.outerWidth(true);o.width(h).height(e.controls.height()).css({top:e.container.height()-e.controls.height()});q.width(e.container.width()-h-fullScreenBtnWidth).height(e.controls.height()).css({top:e.container.height()-e.controls.height(),left:h+fullScreenBtnWidth})};f(document).resize(function(){l()});
...
"hidden")}).delegate("input[type=radio]","click",function(){lang=this.value;if(lang=="none")a.selectedTrack=null;else for(e=0;e<a.tracks.length;e++)if(a.tracks[e].srclang==lang){a.selectedTrack=a.tracks[e];a.captions.attr("lang",a.selectedTrack.srclang);a.displayCaptions();break}});if(a.options.alwaysShowControls)a.container.find(".mejs-captions-position").addClass("mejs-captions-position-hover");else{var g;a.container.bind("mouseenter mouseover focusin",function(){clearTimeout(g);g=null;a.container.find(".mejs-captions-position").addClass("mejs-captions-position-hover")}).bind("mouseleave focusout",
function(){d.paused||g||(g=setTimeout(function(){a.container.find(".mejs-captions-position").removeClass("mejs-captions-position-hover")},1300))})}a.trackToLoad=-1;a.selectedTrack=null;a.isLoadingTrack=false;for(e=0;e<a.tracks.length;e++)a.tracks[e].kind=="subtitles"&&a.addTrackButton(a.tracks[e].srclang,a.tracks[e].label);a.loadNextTrack();d.addEventListener("timeupdate",function(){a.displayCaptions()},false);d.addEventListener("loadedmetadata",function(){a.displayChapters()},false);a.container.hover(function(){if(a.hasChapters){a.chapters.css("visibility",
