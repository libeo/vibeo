...
setControlsSize:function(){var a=0,c=0,b=this.controls.find(".mejs-time-rail"),e=this.controls.find(".mejs-time-total");this.controls.find(".mejs-time-current");this.controls.find(".mejs-time-loaded");var d=b.siblings();if(this.options&&!this.options.autosizeProgress)c=parseInt(b.css("width"));if(c===0||!c){d.each(function(){if(f(this).css("position")!="absolute")a+=f(this).outerWidth(true)});c=this.controls.width()-a-(b.outerWidth(true)-b.width())}b.width(c);e.width(c-(e.outerWidth(true)-e.width()));
this.setProgressRail&&this.setProgressRail();this.setCurrentRail&&this.setCurrentRail()},buildposter:function(a,c,b,e){var d=f('<div class="mejs-poster mejs-layer"></div>').appendTo(b);c=a.$media.attr("poster");if(a.options.poster!=="")c=a.options.poster;c!==""&&c!=null?this.setPoster(c):d.hide();e.addEventListener("play",function(){d.hide()},false)},setPoster:function(a){var c=this.container.find(".mejs-poster"),b=c.find("img");if(b.length==0)b=f('<img width="100%" height="100%" />').appendTo(c);
b.attr("src",a)},buildoverlays:function(a,c,b,e){if(a.isVideo){var d=f('<div class="mejs-overlay mejs-layer"><div class="mejs-overlay-loading"><span></span></div></div>').hide().appendTo(b),g=f('<div class="mejs-overlay mejs-layer"><div class="mejs-overlay-error"></div></div>').hide().appendTo(b),k=f('<div class="mejs-overlay mejs-layer mejs-overlay-play"><div class="mejs-overlay-button"></div></div>').appendTo(b).click(function(){e.paused?e.play():e.pause()});e.addEventListener("play",function(){k.hide();
d.hide();c.find(".mejs-time-buffering").hide();g.hide()},false);e.addEventListener("playing",function(){k.hide();d.hide();c.find(".mejs-time-buffering").hide();g.hide()},false);e.addEventListener("seeking",function(){d.show();c.find(".mejs-time-buffering").show()},false);e.addEventListener("seeked",function(){d.hide();c.find(".mejs-time-buffering").hide()},false);e.addEventListener("pause",function(){mejs.MediaFeatures.isiPhone||k.show()},false);e.addEventListener("waiting",function(){d.show();c.find(".mejs-time-buffering").show()},
false);e.addEventListener("loadeddata",function(){d.show();c.find(".mejs-time-buffering").show()},false);e.addEventListener("canplay",function(){d.hide();c.find(".mejs-time-buffering").hide()},false);e.addEventListener("error",function(){d.hide();c.find(".mejs-time-buffering").hide();g.show();g.find("mejs-overlay-error").html("Error loading this resource")},false)}},buildkeyboard:function(a,c,b,e){f(document).keydown(function(d){if(a.hasFocus&&a.options.enableKeyboard)for(var g=0,k=a.options.keyActions.length;g<
k;g++)for(var i=a.options.keyActions[g],l=0,m=i.keys.length;l<m;l++)if(d.keyCode==i.keys[l]){d.preventDefault();i.action(a,e);return false}return true});f(document).click(function(d){if(f(d.target).closest(".mejs-container").length==0)a.hasFocus=false})},findTracks:function(){var a=this,c=a.$media.find("track");a.tracks=[];c.each(function(b,e){e=f(e);a.tracks.push({srclang:e.attr("srclang").toLowerCase(),src:e.attr("src"),kind:e.attr("kind"),label:e.attr("label")||"",entries:[],isLoaded:false})})},
changeSkin:function(a){this.container[0].className="mejs-container "+a;this.setPlayerSize(this.width,this.height);this.setControlsSize()},play:function(){this.media.play()},pause:function(){this.media.pause()},load:function(){this.media.load()},setMuted:function(a){this.media.setMuted(a)},setCurrentTime:function(a){this.media.setCurrentTime(a)},getCurrentTime:function(){return this.media.currentTime},setVolume:function(a){this.media.setVolume(a)},getVolume:function(){return this.media.volume},setSrc:function(a){this.media.setSrc(a)},
...
i=c.find(".mejs-time-handle"),l=i.find(".mejs-time-handle-rewind"),m=i.find(".mejs-time-handle-forward"),p=c.find(".mejs-time-float"),r=c.find(".mejs-time-float-current"),t=function(h){h=h.pageX;var n=g.offset(),q=g.outerWidth(),s=0;s=0;var u=h-n.left;if(h>n.left&&h<=q+n.left&&e.duration){s=(h-n.left)/q;s=s<=0.02?0:s*e.duration;j&&e.setCurrentTime(s);if(!mejs.MediaFeatures.hasTouch){p.css("left",u);r.html(mejs.Utility.secondsToTimeCode(s));p.show()}}},j=false,o=false;g.bind("mousedown",function(h){if(h.which===
1){j=true;t(h);f(document).bind("mousemove.dur",function(n){t(n)}).bind("mouseup.dur",function(){j=false;p.hide();f(document).unbind(".dur")});return false}}).bind("mouseenter",function(){o=true;f(document).bind("mousemove.dur",function(h){t(h)});mejs.MediaFeatures.hasTouch||p.show()}).bind("mouseleave",function(){o=false;if(!j){f(document).unbind(".dur");p.hide()}});i.focusin(function(){l.removeClass("visuallyhidden");m.removeClass("visuallyhidden")});i.focusout(function(){l.addClass("visuallyhidden");
...
f(document).bind("mouseup",function(){j=false;p.hide()}).bind("mousemove",function(h){if(j||o)t(h)});e.addEventListener("progress",function(h){a.setProgressRail(h);a.setCurrentRail(h)},false);e.addEventListener("timeupdate",function(h){a.setProgressRail(h);a.setCurrentRail(h)},false);d.loaded=b;d.total=g;d.current=k;d.handle=i},setProgressRail:function(a){var c=a!=undefined?a.target:this.media,b=null;if(c&&c.buffered&&c.buffered.length>0&&c.buffered.end&&c.duration)b=c.buffered.end(0)/c.duration;
...
d.options.volumeUpText+"</span></button></div></div>").appendTo(c),i=d.container.find(".mejs-volume-slider"),l=d.container.find(".mejs-volume-total"),m=d.container.find(".mejs-volume-current"),p=d.container.find(".mejs-volume-handle");c=d.container.find(".mejs-volume-plus");b=d.container.find(".mejs-volume-minus");var r=function(h){var n=i.is(":visible");n||i.show();h=Math.max(0,h);h=Math.min(h,1);h==0?k.removeClass("mejs-mute").addClass("mejs-unmute"):k.removeClass("mejs-unmute").addClass("mejs-mute");
if(g=="vertical"){var q=l.height(),s=l.position();h=q-q*h;p.css("top",s.top+h-p.height()/2);m.height(q-h);m.css("top",s.top+h)}else{q=l.width();s=l.position();h=q*h;p.css("left",s.left+h-p.width()/2);m.width(h)}n||i.hide()},t=function(h){var n=null,q=l.offset();if(g=="vertical"){n=l.height();parseInt(l.css("top").replace(/px/,""),10);n=(n-(h.pageY-q.top))/n;if(q.top==0||q.left==0)return}else{n=l.width();n=(h.pageX-q.left)/n}n=Math.max(0,n);n=Math.min(n,1);r(n);n==0?e.setMuted(true):e.setMuted(false);
e.setVolume(n)},j=false,o=false;k.bind("mouseenter",function(){i.show();r(e.volume);o=true}).bind("mouseleave",function(){o=false;j||i.hide()}).children("button").bind("focus",function(){i.show();o=true});c.click(function(){e.setVolume(Math.min(e.volume+0.1,1))});b.click(function(){e.setVolume(Math.max(e.volume-0.1,0))});f(document).on("focus","button,a",{volumeSlider:i},function(h){f(this).closest(k).length==0&&h.data.volumeSlider.hide()});i.bind("mouseover",function(){o=true}).bind("mousedown",
function(h){console.log(h.offsetX);if(h.offsetX!=0){t(h);f(document).bind("mousemove.vol",function(n){t(n)}).bind("mouseup.vol",function(){j=false;f(document).unbind(".vol");!o&&g=="vertical"&&i.hide()});j=true;return false}});k.find(":first").click(function(){e.setMuted(!e.muted)}).focus(function(){f(this).attr("aria-live","polite")}).blur(function(){f(this).attr("aria-live","off")});e.addEventListener("volumechange",function(){var h=k.children("button").children("span");if(!j)if(e.muted){r(0);k.removeClass("mejs-mute").addClass("mejs-unmute");
...
d.container.before('<button class="guard visuallyhidden">Quitter le mode plein \u00e9cran</button>');d.container.after('<button class="guard visuallyhidden">Quitter le mode plein \u00e9cran</button>')}});else{var k=null;c=function(){var j=document.createElement("x"),o=document.documentElement,h=window.getComputedStyle;if(!("pointerEvents"in j.style))return false;j.style.pointerEvents="auto";j.style.pointerEvents="x";o.appendChild(j);h=h&&h(j,"").pointerEvents==="auto";o.removeChild(j);return!!h}();
console.log("supportsPointerEvents",c);if(c&&!mejs.MediaFeatures.isOpera){var i=false,l=function(){if(i){m.hide();p.hide();r.hide();g.css("pointer-events","");d.controls.css("pointer-events","");i=false}},m=f('<div class="mejs-fullscreen-hover" />').appendTo(d.container).mouseover(l),p=f('<div class="mejs-fullscreen-hover"  />').appendTo(d.container).mouseover(l),r=f('<div class="mejs-fullscreen-hover"  />').appendTo(d.container).mouseover(l),t=function(){var j={position:"absolute",top:0,left:0};
m.css(j);p.css(j);r.css(j);m.width(d.container.width()).height(d.container.height()-d.controls.height());j=g.offset().left-d.container.offset().left;fullScreenBtnWidth=g.outerWidth(true);p.width(j).height(d.controls.height()).css({top:d.container.height()-d.controls.height()});r.width(d.container.width()-j-fullScreenBtnWidth).height(d.controls.height()).css({top:d.container.height()-d.controls.height(),left:j+fullScreenBtnWidth})};f(document).resize(function(){t()});g.mouseover(function(){if(!d.isFullScreen){var j=
g.offset(),o=a.container.offset();e.positionFullscreenButton(j.left-o.left,j.top-o.top,false);g.css("pointer-events","none");d.controls.css("pointer-events","none");m.show();r.show();p.show();t();i=true}});e.addEventListener("fullscreenchange",function(){l()})}else{g.click(function(){if(k!==null){clearTimeout(k);delete k}var j=f(".mejs-controls").outerHeight(true);setTimeout(function(){var o=f(window).width(),h=f(window).height()-j;f("#application").css({position:"absolute",width:o,height:h,"z-index":"100",
top:0,left:0});var n=f("#application").offset().left,q=f("#application").offset().top;f("#application").css({"margin-left":"-"+n+"px","margin-top":"-"+q+"px"});d.setPlayerSize(o,h);d.ieFullScreen=true;e.setVideoSize(o,h);e.enterFullScreen()},300);f("body").css({overflow:"hidden"})});g.mouseover(function(){if(k!==null){clearTimeout(k);delete k}var j=g.offset(),o=a.container.offset();e.positionFullscreenButton(j.left-o.left,j.top-o.top,true)}).mouseout(function(){if(k!==null){clearTimeout(k);delete k}k=
setTimeout(function(){e.hideFullscreenButton()},1500)})}}a.fullscreenBtn=g;f(document).bind("keydown",function(j){if((mejs.MediaFeatures.hasTrueNativeFullScreen&&mejs.MediaFeatures.isFullScreen()||d.isFullScreen)&&j.keyCode==27)a.exitFullScreen()})}},enterFullScreen:function(){var a=this;if(!(a.media.pluginType!=="native"&&(mejs.MediaFeatures.isFirefox||a.options.usePluginFullScreen))){docStyleOverflow=document.documentElement.style.overflow;document.documentElement.style.overflow="hidden";normalHeight=
a.container.height();normalWidth=a.container.width();if(a.media.pluginType==="native")if(mejs.MediaFeatures.hasTrueNativeFullScreen){mejs.MediaFeatures.requestFullScreen(a.container[0]);a.isInIframe&&setTimeout(function b(){if(a.isNativeFullScreen)f(window).width()!==screen.width?a.exitFullScreen():setTimeout(b,500)},500)}else if(mejs.MediaFeatures.hasSemiNativeFullScreen){a.media.webkitEnterFullscreen();return}if(a.isInIframe){var c=a.options.newWindowCallback(this);if(c!=="")if(mejs.MediaFeatures.hasTrueNativeFullScreen)setTimeout(function(){if(!a.isNativeFullScreen){a.pause();
...
(function(f){f.extend(mejs.MepDefaults,{startLanguage:"",tracksTextTurnOn:"Sous-titres activ\u00e9s",tracksTextTurnOff:"Sous-titres d\u00e9sactiv\u00e9s"});f.extend(MediaElementPlayer.prototype,{hasChapters:false,buildtracks:function(a,c,b,e){if(a.isVideo)if(a.tracks.length!=0){var d=this,g;a.chapters=f('<div class="mejs-chapters mejs-layer"></div>').prependTo(b).hide();a.captions=f('<div class="mejs-captions-layer mejs-layer"><div class="mejs-captions-position"><span class="mejs-captions-text"></span></div></div>').prependTo(b).hide();
a.captionsText=a.captions.find(".mejs-captions-text");a.captionsButton=f('<div class="mejs-button mejs-captions-button"><button type="button" aria-controls="'+d.id+'" aria-live="polite"><span class="visuallyhidden">'+d.options.tracksTextTurnOn+'</span></button><div class="mejs-captions-selector"><ul><li><input type="radio" name="'+a.id+'_captions" id="'+a.id+'_captions_none" value="none" checked="checked" /><label for="'+a.id+'_captions_none">Aucun</label></li></ul></div></div>').appendTo(c);a.captionsLegend=
...
a;this.captionsButton.find("input[value="+a+"]").prop("disabled",false).siblings("label").html(c);this.options.startLanguage==a&&f("#"+this.id+"_captions_"+a).click();this.adjustLanguageBox()},addTrackButton:function(a,c){if(c==="")c=mejs.language.codes[a]||a;this.captionsButton.find("ul").append(f('<li><input type="radio" name="'+this.id+'_captions" id="'+this.id+"_captions_"+a+'" value="'+a+'" disabled="disabled" /><label for="'+this.id+"_captions_"+a+'" lang="'+a+'">'+c+" (loading)</label></li>"));
...
c.entries.times[a].start&&this.media.currentTime<=c.entries.times[a].stop){this.captionsText.html(c.entries.text[a]);this.captions.show().height(0);return}this.captions.hide()}},displayChapters:function(){var a;for(a=0;a<this.tracks.length;a++)if(this.tracks[a].kind=="chapters"&&this.tracks[a].isLoaded){this.drawChapters(this.tracks[a]);this.hasChapters=true;break}},drawChapters:function(a){var c=this,b,e,d=e=0;c.chapters.empty();for(b=0;b<a.entries.times.length;b++){e=a.entries.times[b].stop-a.entries.times[b].start;
...
(function(f){f.extend(mejs.MepDefaults,{contextMenuItems:[{render:function(a){if(typeof a.enterFullScreen=="undefined")return null;return a.isFullScreen?"D\u00e9sactiver plein \u00e9cran":"Activer plein \u00e9cran"},click:function(a){a.isFullScreen?a.exitFullScreen():a.enterFullScreen()}},{render:function(a){return a.media.muted?"D\u00e9sactiver volume":"Activer volume"},click:function(a){a.media.muted?a.setMuted(false):a.setMuted(true)}},{isSeparator:true},{render:function(){return"T\u00e9l\u00e9charger la vid\u00e9o"},
click:function(a){window.location.href=a.media.currentSrc}}]});f.extend(MediaElementPlayer.prototype,{buildcontextmenu:function(a){a.contextMenu=f('<div class="mejs-contextmenu"></div>').appendTo(f("body")).hide();a.container.bind("contextmenu",function(c){if(a.isContextMenuEnabled){c.preventDefault();a.renderContextMenu(c.clientX-1,c.clientY-1);return false}});a.container.bind("click",function(){a.contextMenu.hide()});a.contextMenu.bind("mouseleave",function(){a.startContextMenuTimer()})},isContextMenuEnabled:true,
enableContextMenu:function(){this.isContextMenuEnabled=true},disableContextMenu:function(){this.isContextMenuEnabled=false},contextMenuTimeout:null,startContextMenuTimer:function(){var a=this;a.killContextMenuTimer();a.contextMenuTimer=setTimeout(function(){a.hideContextMenu();a.killContextMenuTimer()},750)},killContextMenuTimer:function(){var a=this.contextMenuTimer;if(a!=null){clearTimeout(a);delete a}},hideContextMenu:function(){this.contextMenu.hide()},renderContextMenu:function(a,c){for(var b=
this,e="",d=b.options.contextMenuItems,g=0,k=d.length;g<k;g++)if(d[g].isSeparator)e+='<div class="mejs-contextmenu-separator"></div>';else{var i=d[g].render(b);if(i!=null)e+='<div class="mejs-contextmenu-item" data-itemindex="'+g+'" id="element-'+Math.random()*1E6+'">'+i+"</div>"}b.contextMenu.empty().append(f(e)).css({top:c,left:a}).show();b.contextMenu.find(".mejs-contextmenu-item").each(function(){var l=f(this),m=parseInt(l.data("itemindex"),10),p=b.options.contextMenuItems[m];typeof p.show!="undefined"&&
p.show(l,b);l.click(function(){typeof p.click!="undefined"&&p.click(b);b.contextMenu.hide()})});setTimeout(function(){b.killControlsTimer("rev3")},100)}})})(mejs.$);
