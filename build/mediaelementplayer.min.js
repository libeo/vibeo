...
a.captionsButton.find("span");a.captionsButton.children("button").click(function(){if(a.selectedTrack!==null){a.selectedTrack=null;a.captionsLegend.text(d.options.tracksTextTurnOn);a.captionsText.empty();f("#"+a.id+"_captions_none").attr("checked","checked");f(this).parent().removeClass("activeButton")}else{lang=a.captions.attr("lang")||a.options.startLanguage;f(this).parent().addClass("activeButton");for(g=0;g<a.tracks.length;g++)if(a.tracks[g].srclang==lang){a.selectedTrack=a.tracks[g];a.captions.attr("lang",
a.selectedTrack.srclang);a.displayCaptions();f("#"+a.id+"_captions_"+lang).attr("checked","checked");break}a.captionsLegend.text(d.options.tracksTextTurnOff)}});a.options.showCaptionPopup&&a.captionsButton.hover(function(){f(this).find(".mejs-captions-selector").css("visibility","visible")},function(){f(this).find(".mejs-captions-selector").css("visibility","hidden")}).delegate("input[type=radio]","click",function(){lang=this.value;if(lang=="none")a.selectedTrack=null;else for(g=0;g<a.tracks.length;g++)if(a.tracks[g].srclang==
...
this,e="",d=b.options.contextMenuItems,g=0,k=d.length;g<k;g++)if(d[g].isSeparator)e+='<div class="mejs-contextmenu-separator"></div>';else{var i=d[g].render(b);if(i!=null)e+='<div class="mejs-contextmenu-item" data-itemindex="'+g+'" id="element-'+Math.random()*1E6+'">'+i+"</div>"}b.contextMenu.empty().append(f(e)).css({top:c,left:a}).show();b.contextMenu.find(".mejs-contextmenu-item").each(function(){var l=f(this),m=parseInt(l.data("itemindex"),10),o=b.options.contextMenuItems[m];typeof o.show!="undefined"&&
o.show(l,b);l.click(function(){typeof o.click!="undefined"&&o.click(b);b.contextMenu.hide()})});setTimeout(function(){b.killControlsTimer("rev3")},100)}})})(mejs.$);
